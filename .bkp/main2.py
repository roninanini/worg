...
print("Leitura ADC:", PLANT_1.read(), PLANT_2.read(), PLANT_3.read(), PLANT_4.read())
...
while True:
    ...
    if wlan.isconnected():
        try:
            ...
            # SENSORES DE SOLO
            try:
                soil_1 = str(PLANT_1.read())
            except Exception as e:
                soil_1 = '101'
            try:
                soil_2 = str(PLANT_2.read())
            except Exception as e:
                soil_2 = '101'
            try:
                soil_3 = str(PLANT_3.read())
            except Exception as e:
                soil_3 = '101'
            try:
                soil_4 = str(PLANT_4.read())
            except Exception as e:
                soil_4 = '101'
    ...
    try:
        ...
        if io.temp() < 18:
            ...
            io.fan_2(0)
            try:
                client_mqtt.publish('worg/status/fan_1', f'{{"Fan 01": {"1" if mcp.pin(15) else "0"}}}', retain=True,qos=1)
                client_mqtt.publish('worg/status/fan_2', f'{{"Fan 02": {"1" if mcp.pin(14) else "0"}}}', retain=True,qos=1)
            except Exception as e:
                pass
        elif 18 <= io.temp() < 22:
        ...
        if io.vpd() < 0.8:
            ...
            io.humidifier(0)
            try:
                client_mqtt.publish('worg/status/humidifier', f'{{"Humidifier": {"1" if mcp.pin(12) else "0"}}}',retain=True, qos=1)
                client_mqtt.publish('worg/status/deshumidifier', f'{{"Deshumidifier": {"1" if mcp.pin(11) else "0"}}}',retain=True, qos=1)
            except Exception as e:
                pass
        elif 0.8 <= io.vpd() < 1.2:
            io.deshumidifier(0)
        ...
        if 10 <= HOUR < 16:
            io.lighting(0)
            try:
                client_mqtt.publish('worg/status/lighting', f'{{"Lighting": {"1" if mcp.pin(13) else "0"}}}',retain=True, qos=1)
            except Exception as e:
                pass
        else:
            io.lighting(1)
