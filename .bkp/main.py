...
while True:
    ...
    if wlan.isconnected():
        try:
            ...
            client_mqtt.publish('worg/status/water_pump4', f'{{"Water Pump 04": {"1" if mcp.pin(0) else "0"}}}',
                                retain=True, qos=1)
            client_mqtt.publish('worg/status/lighting', f'{{"Lighting": {"1" if mcp.pin(13) else "0"}}}', retain=True,
                                qos=1)
            ...
            client_mqtt.publish('worg/status/humidifier', f'{{"Humidifier": {"1" if mcp.pin(12) else "0"}}}',
                                retain=True, qos=1)
            client_mqtt.publish('worg/status/deshumidifier', f'{{"Deshumidifier": {"1" if mcp.pin(11) else "0"}}}',
                                retain=True, qos=1)
    ...
    try:
        ...
        print(io.soil_1(), io.soil_2(), io.soil_3(), io.soil_4())
